...
std::string wstr_to_str(const std::wstring& arg)
{
	...
	size_t tSize;
	wcstombs_s(&tSize, const_cast<char*>(res.data()), res.length(), arg.c_str(), arg.length());
    return res;
}
...
void CIniFileA::Load( std::istream& input , const std::string& section, const std::string& key, bool bMerge )
{
    if( !bMerge )
        RemoveAllSections();

    CIniSectionA* pSection = nullptr;
    std::string sRead;
    ...
    while( std::getline( input , sRead ) )
    {
        ...
        if( !sRead.empty() )
        {
			...
			nType = ( (nType == OTHER) && (( sRead.find_first_of("#") == 0) || ( sRead.find_first_of("//") == 0))) ? COMMENT : nType ;
...
#endif
				{					
					std::string sectionValue = sRead.substr( 1 , sRead.size() - 2 );
					if(!section.compare("") || !section.compare(sectionValue))
					{						
            ...
            case KEY:
            {
                ...
                {					
					...
					std::string sKey;
#if 0
					Trim(sRead.substr(0,iFind), sKey);
#else
                    std::string sSub = sRead.substr(0, iFind);
                    Trim(sSub, sKey);
#endif

					std::string sValue;
#if 0
					Trim(sRead.substr(iFind + 1), sValue);
#else
                    sSub = sRead.substr(iFind + 1);
                    Trim(sSub, sValue);
#endif
...
bool CIniFileA::Load(const std::string& fileName , const std::string& section, const std::string& key, bool bMerge )
{
#ifdef _CINIFILE_DEBUG
    std::cout <<  "CIniFileA::Load() - " << fileName << std::endl;
#endif   
	
	//FILE *fp = fopen(fileName.c_str(), "rb");		//FILE *fp = fopen(fileName.c_str(), "rt");	#fread 함수 사용시 마지막 문자 짤림 현상으로 fopen 함수 사용시 "rt" 모드에서 "rb" 모드로 수정 20151104 - jhlee
	FILE *fp = nullptr;
...
{
    CIniSectionA bogus(nullptr,sSection);
...
{
    CIniSectionW bogus(nullptr,sSection);
    return m_sections.find( &bogus );
}
