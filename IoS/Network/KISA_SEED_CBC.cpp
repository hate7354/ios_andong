...
void KISA_SEED_Encrypt_Block_forCBC(unsigned int* in, unsigned int* out, KISA_SEED_KEY* ks)
{
    ...
    uint32_t* K = ks->key_data;	// Pointer of round keys
...
//  uint32_t* data = (uint32_t*)malloc(sizeof(uint32_t) * len);
    uint32_t* data = new uint32_t[len];

    for (i = 0; i < len; i++)
    {
        data[i] = ((uint32_t*)in)[i];
    }
...
//  uint8_t* data = (unsigned char*)malloc(sizeof(unsigned char) * nLen);
    uint8_t* data = new uint8_t[nLen];

#ifndef BIG_ENDIAN
    for (i = 0; i < nLen; i++)
    {
        data[i] = (uint8_t)(in[i / 4] >> ((i % 4) * 8));
    }
#else
    for (i = 0; i < inLen; i++)
    {
        data[i] = (uint8_t)(in[i / 4] >> ((3 - (i % 4)) * 8));
    }
#endif
    return data;
}
...
int SEED_CBC_init(OUT KISA_SEED_INFO* pInfo, IN KISA_ENC_DEC enc, IN BYTE* pbszUserKey, IN BYTE* pbszIV)
{
    ...
    A = ((uint32_t*)pbszUserKey)[0];
    B = ((uint32_t*)pbszUserKey)[1];
    C = ((uint32_t*)pbszUserKey)[2];
    D = ((uint32_t*)pbszUserKey)[3];
...
int SEED_CBC_Encrypt(IN BYTE* pbszUserKey, IN BYTE* pbszIV, IN BYTE* pbszPlainText, IN int nPlainTextLen, OUT BYTE* pbszCipherText)
{
    ...
    uint32_t* outbuf;
    uint32_t* data;
    uint8_t* newpbszPlainText;
    uint8_t* cdata;
    int outlen = 0;
    int nRetOutLeng = 0;
    int nPaddingLeng = 0;
    int nPlainTextPadding = (BLOCK_SIZE_SEED - (nPlainTextLen % BLOCK_SIZE_SEED));
    newpbszPlainText = new uint8_t[nPlainTextLen + nPlainTextPadding];
...
int SEED_CBC_Decrypt(IN uint8_t* pbszUserKey, IN uint8_t* pbszIV, IN uint8_t* pbszCipherText, IN int nCipherTextLen, OUT uint8_t* result)
{
    KISA_SEED_INFO info;
    int outlen = 0;
    int nRetOutLeng = 0;
...
//  uint8_t* newpbszCipherText = (unsigned char*)malloc(sizeof(uint8_t) * (nCipherTextLen));
    uint8_t* newpbszCipherText = new uint8_t[nCipherTextLen];
    ::memcpy(newpbszCipherText, pbszCipherText, nCipherTextLen);

//  uint8_t* pbszPlainText = (uint8_t*)malloc(sizeof(uint8_t) * (nCipherTextLen));
...
//  uint32_t* outbuf = (uint32_t*)malloc(sizeof(uint32_t) * outlen);
    uint32_t* outbuf = new uint32_t[outlen];

    uint32_t* data = chartoint32_for_SEED_CBC(newpbszCipherText, nCipherTextLen);
    delete[] newpbszCipherText;

    SEED_CBC_Process(&info, data, nCipherTextLen, outbuf, &nRetOutLeng);
    if (SEED_CBC_Close(&info, outbuf + (nRetOutLeng) / 4, &nPaddingLeng) > 0) {
        uint8_t* cdata = int32tochar_for_SEED_CBC(outbuf, nRetOutLeng - nPaddingLeng);
